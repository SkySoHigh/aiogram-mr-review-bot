...
class TaskService:
    ...
    def set_task_reviewer(
        ...
        reviewer_id: int,
        reviewer_name: str,
        taken_on_review_at: datetime.datetime,
        reply_msg_id: int,
    ) -> TasksModel:
        self.__client.tasks.update_by(
            values={
    ...
    def reject_task_review(
        self, task_id: int, rejected_from_final_review_at: datetime.datetime
    ) -> TasksModel:
        self.__client.tasks.update_by(
            values={
    ...
    def resubmit_task_to_review_after_fix(self, task_id: int) -> TasksModel:
        self.__client.tasks.update_by(
            values={"status": TaskStates.REVIEW_AFTER_FIX}, id=task_id
        )
        return self.get_task_by_id(task_id=task_id)

    def submit_task_to_final_review(
        self, task_id: int, submitted_to_final_review_at: datetime.datetime
    ) -> TasksModel:
        self.__client.tasks.update_by(
            values={
                "submitted_to_final_review_at": submitted_to_final_review_at,
    ...
    def accept_final_task_review(
        self, task_id: int, final_reviewer_name: str, completed_at: datetime.datetime
    ) -> TasksModel:
        self.__client.tasks.update_by(
            values={
    ...
    def reject_final_task_review(
        self, task_id: int, rejected_from_final_review_at: datetime.datetime
    ) -> TasksModel:
        self.__client.tasks.update_by(
            values={
                "submitted_to_final_review_at": None,
                "rejected_from_final_review_at": rejected_from_final_review_at,
                "status": TaskStates.FIX_REQUIRED,
            },
            id=task_id,
    ...
    def count_tasks_on_review(self, chat_id: int, reviewer_id: int = None) -> int:
        ...
        if reviewer_id:
            return self.__client.tasks.get_count_by_status_with_filter(
                status=statuses, reviewer_id=reviewer_id, chat_id=chat_id
            )
        else:
            return self.__client.tasks.get_count_by_status_with_filter(
                status=statuses,
    ...
    def set_reply_msg_id(self, task_id: int, reply_msg_id: int):
        return self.__client.tasks.update_by(
            values={"reply_msg_id": reply_msg_id},
            id=task_id,
        )
